...
if ( _show ) then {
    ...
    if ( _showNotification < 0 || _showNotification > 0 ) then {
        ...
        _buttonOK setVariable [ "VA_showNotification_ctrls", [ _ctrlSText, _ctrlGrp ] ];
        ...
        _buttonOK ctrlAddEventHandler [ "buttonClick", {
            ...
            [ ctrlParent _buttonOK, "", false ] call VA_fnc_showRestrictedItems;            
        }];
        ...
        _display displayAddEventHandler ["keyDown",{
            ...
            switch ( true ) do {
                ...
                case ( _keyCode == DIK_NUMPADENTER ) : {
                    [ _this select 0, '', false ] call VA_fnc_showRestrictedItems;
                };
            };
        }];
        
        if ( _showNotification > 0 ) then {
            VA_overrideVA_threadMsg = [ _display, _showNotification ] spawn {
                ...
                with uiNamespace do {
                    [ _display, '', false ] call VA_fnc_showRestrictedItems;
                };
                VA_overrideVA_thread = nil;
            };
        };    
    };
    ...
    _buttonOK = _display displayCtrl IDC_RSCDISPLAYARSENAL_TEMPLATE_BUTTONOK;
    _restrictedCtrls = _buttonOk getVariable "VA_showNotification_ctrls";
    if !( isNil "_restrictedCtrls" ) then {
        ...
        _thread = uiNamespace getVariable "VA_overrideVA_threadMsg";
        if ( !isNil "_thread" && !scriptDone _thread ) then {
            terminate _thread;
        };
        
        _buttonOK setVariable [ "VA_showNotification_ctrls", nil ];
        _buttonOK ctrlAddEventHandler ["buttonclick","with uinamespace do {[ctrlparent (_this select 0)] call VA_fnc_overrideVATemplateOK;};"];
        
        [ _display ] call VA_fnc_addVAKeyEvents;
