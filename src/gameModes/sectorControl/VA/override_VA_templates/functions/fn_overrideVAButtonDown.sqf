...
#define IDCS_LEFT\
    ...
    IDC_RSCDISPLAYARSENAL_TAB_BINOCUVA,\
...
_fullVersion = missionnamespace getvariable ["BIS_fnc_arsenal_fulVAenal",false];
...
switch true do {
    ...
    case (_key in [DIK_RETURN,DIK_NUMPADENTER]): {
        ...
        if (ctrlFade _ctrlTemplate == 0) then {
            if (BIS_fnc_arsenal_type == 0) then {
                [_display] call VA_fnc_overrideVATemplateOK;
            } else {
    ...
    //--- Open
    case (_key == DIK_O): {
        if (_ctrl) then {['buttonLoad',[_display]] call bis_fnc_arsenal;[_display] call VA_fnc_applyLBColors;};
    };
    //--- Randomize
    case (_key == DIK_R && getMissionConfigValue[ "VA_overrideVA_random", 0 ] isEqualTo 0 ): {
        if (_ctrl) then {
            if (BIS_fnc_arsenal_type == 0) then {
                if (_shift) then {
                    _soldiers = [];
