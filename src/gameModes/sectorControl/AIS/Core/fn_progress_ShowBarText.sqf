...
_isRcs = if ((count _this) > 2) then {_this select 2} else {false};

disableSerialization;
private _progressTitle = uiNamespace getVariable (["AIS_Core_Progress_ProgressTitle", "AIS_Core_Progress_ProgressTitleRsc"] select _isRcs);
